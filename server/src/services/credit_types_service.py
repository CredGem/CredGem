...
async def create_credit_type(
    ...
    async with db_session() as session_ctx:
        result = await credit_types_db.create_credit_type(
            db=session_ctx.session, credit_type_request=credit_type_request
        )
...
async def update_credit_type(
    credit_type_request: UpdateCreditTypeRequest, credit_type_id: str
) -> CreditTypeResponse:
    async with db_session() as session_ctx:
...
async def delete_credit_type(credit_type_id: str) -> None:
    async with db_session() as session_ctx:
